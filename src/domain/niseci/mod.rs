...
pub(crate) struct RecordNISECI {
  ...
  pub(crate) lunghezza: u32, /// in millimetri
...
impl RisultatoNISECI {
    pub(crate) fn new(valore: Option<f32>, rqe: Option<f32>, valori_intermedi: ValoriIntermediNISECI) -> Self {
        Self {
            valore,
            rqe,
            valori_intermedi
...
impl InfoPopolazioniNISECI {
  ...
  pub(crate) fn get_info_pop(map: &HashMap<String, ClassiEtaSpecieNISECI>) -> Result<InfoPopolazioniNISECI, Vec<String>> {
...
impl InfoPopolazioniAlieneNISECI {
  ...
  pub(crate) fn get_info_pop_aliene(classi_eta: &ClassiEtaAlieniNISECI) -> Result<InfoPopolazioniAlieneNISECI, Vec<String>> {
...
impl EsemplariPerCattura {
  pub(crate) fn new_prevalorized(numero_passaggio: u8, specie: &SpecieNISECI) -> EsemplariPerCattura {
