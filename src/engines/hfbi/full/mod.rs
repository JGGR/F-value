...
mod full_hfbi_private_tests {
    ...
    fn test_mmi_and_hfbi() {
        ...
        assert!(mmi_result.is_ok(), "calculate_mmi should succeed with non-empty data");

        let (mmi, intermediates) = mmi_result.unwrap();

        let tested: f32 = 0.551520581; // ho testato a mano il risultato

        assert!(mmi.is_finite(), "MMI should be a finite number, but was {}", mmi);
        ...
        assert!(hfbi.is_finite(), "HFBI should be a finite number, but was {}", hfbi);
    }
}
